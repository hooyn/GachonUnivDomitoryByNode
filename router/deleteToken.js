    ...
    connection.query('select * from user where id=?',[curUser],function(err, rows){
        ...
        if(rows[0]){
          connection.query('update user set token=null where id=?',[curUser],function(err, rows){
              if(err) throw err;
              connection.query('update replylist set token=null where userId=?',[curUser],function(err,rows){
                if(err) throw err;
              })
              connection.query('update articlelist set token=null where userId=?',[curUser],function(err,rows){
                if(err) throw err;
              })
